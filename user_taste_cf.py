...
wine=pd.read_csv('wine_list.csv')
...
# 사용자의 취향을 리스트로 넘겨주기
new_data = [' ', 'User Wine Taste', ' ', '스파클링', ' ', 4, 2, ' ', '레몬', '사과', '꽃']
wine.loc[len(wine)] = new_data

#유효하게 사용되어질 컬럼들을 더해서 만든 'feature'

#와인이름 빈공간 없이 만들기
# wine['wineName'] = wine['wineName'].str.replace(' ','')

#int -> str 바꾸기 
wine['winePrice'] = wine['winePrice'].astype(str)
wine['wineSweet'] = wine['wineSweet'].astype(str)
wine['wineBody'] = wine['wineBody'].astype(str)

#키워드 특징 모두 문자열로 합치기
wine['keyword'] = wine['wineType'] + ' ' + wine['wineSweet'] + '당도 ' + wine['wineBody'] + '바디 ' + wine['aroma1'] + ' ' + wine['aroma2'] + ' ' + wine['aroma3']
...
tfidf_matrix = tf.fit_transform(wine['keyword'])
tfidf_matrix
...
wine_index = pd.Series(data=wine.index, index=wine['wineName'])
wine_index.head()
...
get_recommed_wine('User Wine Taste') #여기에 자신의 먹어본/저장한 와인을 넣으면 그 와인과 비슷한 와인을 추천해줍니다!

#무엇이 비슷한지 한 번 확인해보기
# wine.loc[[207, 1290, 2201, 2224, 1800, 318, 320, 372, 2359, 2329], ['wineName', 'keyword']]
